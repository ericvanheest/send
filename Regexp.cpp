...
// Modified to be std::wstring based (mbcs single-byte only)
...
//#pragma warning( disable : 4049 )   // locally defined symbols imported
...
const std::wstring & CRegErrorHandler::GetErrorString() const
{
	return m_szError;
}
...
public:
	...
	std::wstring GetReplaceString( const TCHAR* sReplaceExp ) const;
...
#ifdef _DEBUG
	std::wstring m_originalPattern;
	std::wstring m_modifiedPattern;
#endif
};
...
std::wstring Regexp::GetReplaceString( LPCTSTR source ) const
{
	ClearErrorString();
	if ( rc )
...
const std::wstring Regexp::operator[]( unsigned int i ) const
{
	ClearErrorString();
//	ASSERT( rc );
	if ( rc )
...
std::wstring regexp::GetReplaceString( const TCHAR* sReplaceExp ) const
{
	std::wstring szEmpty( _T( "" ) );
	...
	std::wstring szReplace;

	buf = new TCHAR[replacelen];
	...
	while ((c = *src++) != _T('\0')) 
	{
					...
					endp[no] > startp[no]) 
		{
			// Get tagged expression
			len = endp[no] - startp[no];
			_tcsncpy_s(buf, replacelen, startp[no], len);
			buf += len;
			if (len != 0 && *(buf-1) == _T( '\0' ))
			{	/* strncpy hit NUL. */
...
std::wstring Regexp::GetErrorString() const
{
	// make sure that if status == 0 that we have an error string
	if (( rc ? rc->GetErrorString() : m_szError).length() != 0)
		return std::wstring(_T("No error text available."));
